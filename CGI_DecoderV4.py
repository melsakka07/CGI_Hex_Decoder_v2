...
def decode_cdr_cgi(hex_str):
    if not hex_str:
        return {
            'CDR CGI (Hex)': ('0000000000', 10),
            'CDR CGI (Dec)': ('00000000000', 11)
        }

    # Skip the first 5 digits
    cdr_hex = hex_str[5:]

    # Convert CDR CGI to decimal
    cdr_dec = int(cdr_hex, 16)

    # Format CDR CGI with leading zeros
    cdr_dec_formatted = f'{cdr_dec:011d}'

    return {
        'CDR CGI (Hex)': (cdr_hex, len(cdr_hex)),
        'CDR CGI (Dec)': (cdr_dec_formatted, len(cdr_dec_formatted))
    }

def decode_cgi():
    hex_4g_cgi = entry_4g.get()
    hex_5g_cgi = entry_5g.get()
    hex_cdr_cgi = entry_cdr.get()

    decoded_4g_cgi = decode_4g_cgi(hex_4g_cgi)
    decoded_5g_cgi = decode_5g_cgi(hex_5g_cgi)
    decoded_cdr_cgi = decode_cdr_cgi(hex_cdr_cgi)
    ...
    # Display CDR results
    result_cdr.delete(1.0, tk.END)
    result_cdr.insert(tk.END, "CDR CGI Breakdown:\n")
    for key, (value, digits) in decoded_cdr_cgi.items():
        result_cdr.insert(tk.END, f'{key}: {value} (Digits: {digits})\n')

def clear_entries():
    entry_4g.delete(0, tk.END)
    entry_5g.delete(0, tk.END)
    entry_cdr.delete(0, tk.END)
    result_4g.delete(1.0, tk.END)
    result_5g.delete(1.0, tk.END)
    result_cdr.delete(1.0, tk.END)
...
def export_results():
    ...
    if file_path:
        with open(file_path, 'w') as file:
            ...
            file.write("\nMediation-CDR-Cell-Information Breakdown:\n")
            file.write(result_cdr.get(1.0, tk.END))

# Create the main window
root = tk.Tk()
root.title("CGI Decoder Hex to Dec")
root.geometry("545x870")
...
ttk.Label(root, text="Enter 5G (Mediation-CDR-Cell-Information) in hex:").grid(row=4, column=0, padx=10, pady=5, sticky="e")
entry_cdr = ttk.Entry(root, width=35)
entry_cdr.grid(row=4, column=1, padx=10, pady=5, columnspan=2)
...
decode_button.grid(row=5, column=0, pady=10, padx=(20, 10))  # Shifted left

clear_button = ttk.Button(root, text="Clear", command=clear_entries, style="Clear.TButton")
clear_button.grid(row=5, column=1, pady=10, padx=10)

export_button = ttk.Button(root, text="Export", command=export_results, style="Export.TButton")
export_button.grid(row=5, column=2, pady=10, padx=10)

# Add results display
result_4g = tk.Text(root, height=10, width=65)
result_4g.grid(row=6, column=0, columnspan=3, padx=10, pady=5)

result_5g = tk.Text(root, height=10, width=65)
result_5g.grid(row=7, column=0, columnspan=3, padx=10, pady=5)

result_cdr = tk.Text(root, height=10, width=65)
result_cdr.grid(row=8, column=0, columnspan=3, padx=10, pady=5)

# Add note
ttk.Label(root, text="Created by M. ElSakka - 2024, All Rights Reserved ©️", font=("Helvetica", 8)).grid(row=9, column=0, columnspan=3, pady=10)
